...
interface Props {
  blog?: Blog | null; // make it optional / nullable
  onClose: () => void;
  onSave: (newBlog: Blog) => void;
}

const BlogAdd: React.FC<Props> = ({ blog, onClose, onSave }) => {
  const [title, setTitle] = useState(blog?.title || "");
  const [content, setContent] = useState(blog?.content || "");
  const [image, setImage] = useState(blog?.image || "");
  const [date] = useState(blog?.date || new Date().toISOString().split("T")[0]);
      ...
      <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-5/12 flex flex-col gap-4 p-4">
          ...
          <div className="flex flex-col gap-4 mt-2">
            <div>
              <Label htmlFor="title" className="text-dark text-lg font-semibold">
                Title
              </Label>
        ...
        <div className="lg:w-7/12 p-6 flex flex-col gap-4 overflow-y-auto max-h-[65vh] pe-4 ps-4 pb-4">
          <Label htmlFor="description" className="text-dark text-lg font-semibold pt-4">
            Description
          </Label>
